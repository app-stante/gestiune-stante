...
// ======================================================
// INCHIDE TOATE LISTELE AUTOCOMPLETE
// ======================================================

function inchideAutocomplete() {

    campuriFiltru.forEach(id => {

        document
            .getElementById(id + "_autocomplete")
            .innerHTML = "";
...
function actualizeazaAutocomplete(idCamp) {

    const valoareScrisa = citesteCamp(idCamp);

    const autocompleteDiv =
        document.getElementById(idCamp + "_autocomplete");

    inchideAutocomplete();


    if (valoareScrisa.length === 0) {
        return;
    }
    ...
    const valoriGasite = [
        ...new Set(
            toateStantele
                .map(stanta => stanta[idCamp] || "")
                .filter(valoare =>
                    valoare.toLowerCase().includes(valoareScrisa)
                )
        )
    ];


    valoriGasite.forEach(valoare => {

        const div = document.createElement("div");

        div.className = "autocomplete-item";

        div.innerText = valoare;


        div.addEventListener("click", () => {

            document.getElementById(idCamp).value = valoare;
